...
Jii.defineClass('Jii.comet.client.NeatClient', /** @lends Jii.comet.client.NeatClient.prototype */{
    ...
    /**
     * @type {Jii.comet.client.ControllerClient}
     */
    comet: null,

    /**
     * @type {NeatComet.SimpleCollectionMapperClient}
     */
    data: null,
    ...
    init: function () {
        ...
        this.data = new NeatComet.SimpleCollectionMapperClient({
            createCollection: _.bind(this._createCollection, this)
        });

        this._neat = new NeatComet.NeatCometClient({
            comet: this,
            getCollection: function(profile, bindingId, openedProfileId) {

                return new NeatComet.adapters.backbone.NeatCometBackboneCollectionAdapter({
                    collection: this.data.get(profile, bindingId),
                    idMapper: function(attributes) {
                        return attributes.id || attributes.uid;
                    }
                });
            }.bind(this)
        });
    ...
    open: function(profiles) {
        return new Promise(function(resolve) {
            this._neat.open(profiles, function(profiles, profileId, openedProfileId) {
                resolve(profiles, profileId, openedProfileId);
            });
        }.bind(this));
    },
